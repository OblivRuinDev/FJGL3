...
public class IOURingNAPI extends Struct<IOURingNAPI> implements NativeResource {
    ...
    static ByteBuffer npad(long struct) { return memByteBuffer(struct + IOURingNAPI.PAD, 3); }
    static byte npad(long struct, int index) {
        return memGetByte(struct + IOURingNAPI.PAD + check(index, 3) * 1);
    }
    static long nresv(long struct) { return memGetLong(struct + IOURingNAPI.RESV); }
    ...
    static void nresv(long struct, long value) { memPutLong(struct + IOURingNAPI.RESV, value); }
